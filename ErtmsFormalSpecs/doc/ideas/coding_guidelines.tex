...
\item Do not create a range that only consists of special values. Use an enumeration (see section \ref{sec:enumeration}) instead. \ruleauthor{MD}{16/01/2014}
\end{rules}
...
\chapter{Model}
\MDfatal{TODO}\MDexplain{add naming conventions for procedures, functions etc.; see also rule \#3 of the committer checklist.}
...
\begin{rules}
\item \label{rule:functions_cases_general_trivialexpressions}\MDwarning{TBD} Cases shall only contain trivial expressions as their return values (i.e. all logic shall be defined inside pre-conditions). \ruleauthor{MD}{31/10/2013}
\item \MDwarning{TBD} Complex return expressions and pre-conditions shall not be used together. \MDnote{this rule is mutually exclusive to \ref{rule:functions_cases_general_trivialexpressions}} \ruleauthor{MD}{31/10/2013}
\end{rules}
...
\item \MDnote{redundant}\MDexplain{Basically we have the same information available through the blame mode. Still I find it a lot easier to have the name of the other right next to the actual comment.} Comments shall always end with \literally{\#$<$github_username$>$, yy/mm/dd}. Where \literally{$<$github_username$>$} is to be replaced with the actual username of the comment-author and \literally{yy/mm/dd} with the current date. \ruleauthor{MD}{12/11/2013}
\end{rules}
...
Before you start working make sure so have a recent copy of the model in your local fork of the \gls{efs} repository. \ruleauthor{MD}{21/01/2014}
...
If you implemented any requirements make sure to mark their respective nodes in the specification view with \literally{Implementation Status: Implemented}.\\ \literally{Reviewed: True} also has to be set if you verified the requirement text matches with that of the original word document of subset-026 \cite{subset26}. \ruleauthor{MD}{14/12/2013}
